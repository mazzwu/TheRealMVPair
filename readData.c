...
BSTNode getTree(BSTNode treeNode, Queue q)
{
	...
	while(!emptyQueue(q)) {
		...
		while(strcmp(buffer, "Section-2") != 0) {
			fscanf(fp, "%s", buffer);	        //store first word after header
			fscanf(fp, "%s", buffer);
		}
		...
		while((strcmp(buffer, "#end") != 0)) {
			...
			if(flag == 1) {
				treeNode = insertBSTree(treeNode, normalise(buffer), normalise(curr));
			} else {
				insertBSTree(treeNode, normalise(buffer), normalise(curr));
				flag = 0;
			}
